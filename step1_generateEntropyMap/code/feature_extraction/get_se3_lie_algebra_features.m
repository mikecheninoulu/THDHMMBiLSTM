function [features] = get_se3_lie_algebra_features(joint_locations, body_model, n_desired_frames, option)
    ...
    lie_algebra_dim = 6;
    %matrix_dim = 3;%only rotation
    matrix_dim = 4; % rotation ans translation
    
    n_given_frames = size(joint_locations, 3);
    
    %SE = get_only_rotations_in_local_coordinates(joint_locations(:,:,1), body_model, option);
    ...
    for n = 1:n_given_frames
        %SE = get_only_rotations_in_local_coordinates(joint_locations(:,:,n), body_model, option);
